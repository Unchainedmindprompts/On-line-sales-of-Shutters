...
export default function HeroSlideshow(){
 ...
 {slides.map(([file,alt,fit],i)=>requested.includes(i)&&<div key={file} className={'hero-slide '+(i===current?'active':'')} aria-hidden={i!==current} role="group" aria-roledescription="slide" aria-label={`${i+1} of ${slides.length}`}><Image src={'/images/shutters/'+file} alt={alt} fill sizes="(max-width: 760px) 90vw, 45vw" priority={i===0} loading={i===0?undefined:'eager'} style={{objectFit:fit}} onLoad={()=>setLoaded(p=>p.includes(i)?p:[...p,i])}/></div>)}
